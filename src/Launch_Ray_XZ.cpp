...
//Launch_Ray_XZ Array Declaration
  ...
  for(int i = 0;i < nz;i++)
  {
    if(z_init - z[i] <= (dz + 1e-11) && z_init - z[i] >= -1*(dz + 1e-11) )
    {
  ...
  for(int i = 0;i < nx;i++)
  {
    ...
    double initX = x_init;
    double currX = x[i];
    if(initX - currX <= dx && initX - currX >= -1*dx )
    {
      thisx_0=i;
      break;
    }
  ...
  double crossxlast = x_init;
  double crosszlast = z_init;
  double deltapos = 1e-9;
  double crossxlast_twin, crosszlast_twin;
  if(fmin(abs(x_init - xmin), abs(x_init - xmax))  < fmin(abs(z_init - zmin), abs(z_init - zmax)))
  {
    //starting at x_init
    crossxlast_twin = x_init;
    crosszlast_twin = z_init + deltapos;
  }else
  {
  //starting at x_init
    crossxlast_twin = x_init + deltapos;
    crosszlast_twin = z_init;
  }
  double twinvx = myvxprev;
  double twinvz = myvzprev;
  double twinx = crossxlast_twin;
  double twinz = crosszlast_twin;
  //__________Time Stepping__________
    ...
    for(int i = 1; i < nt;i++)
    {
      int discreteX = ((twinx-xmin)/dx);
      int discreteZ = ((twinz-zmin)/dz);

      int twinxind = discreteX + (twinx-(discreteX+xmin)*dx > dx/2);
      int twinzind = discreteZ + (twinz-(discreteZ+zmin)*dz > dz/2);
      ...
      double forcext = pow(c,2.0)/(2.0*ncrit)*vec2D(dedendx,twinxind,twinzind, nz);
      double forcezt = pow(c,2.0)/(2.0*ncrit)*vec2D(dedendz,twinxind,twinzind, nz);
     ...
     /* //determining the current x index of the ray
      ...
      discreteX = ((myx-xmin)/dx);
      discreteZ = ((myz-zmin)/dz);
      ...
      for(int j = thisx_m; j <= thisx_p;j++)
      {
        ...
        if((myx > currx && x_init <= (currx)) || (myx < currx && x_init >= (currx)))
        {
          ...
          if(abs(crossx-lastz)>1.0e-20)
          {
            ...
            double xdiff = (myx-twinx);
            double zdiff = (myz-twinz);
            double thisarea = 1e-9/sqrt(xdiff*xdiff + zdiff*zdiff);
            vec3DW(areas, beam, raynum, numcrossing, nrays, ncrossings, thisarea);
