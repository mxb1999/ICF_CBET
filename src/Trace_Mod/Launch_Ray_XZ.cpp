  ...
  while((currX < nx && currZ < nz) && (currX >= 0 && currZ >= 0))
  {
    ...
    if(condx || condz)
    {
      ...
      *boxz = currZ + 1;//[beam][raynum][numcrossing][1]
      ...
      crossesx[beam][raynum][numcrossing] = xnew;    //If the ray spends multiple iterations in the same cell
...
/*  double* nuei = new double[nt]{0.0};
 ...
 // }
  ...
  //__________Time Stepping__________
    ...
    for(int i = 1; i < nt;i++)
    {
      myvz = myvzprev - pow(c,2.0)/(2.0*ncrit)*dedendz[thisx_0][thisz_0]*dt;
        ...
        if((myx > currx && myxprev <= (currx + 1e-10)) || (myx < currx && myxprev >= (currx- 1e-10)))
        {
          ...
          if(abs(crossx-lastz)>1.0e-20)
          {
            ...
            if(myx < (xmax+dx/2 + 1e-10) && myx > (xmin-dx/2 - 1e-10))
            {
              ...
              int* boxx = vec4D(boxes, beam,raynum,numcrossing,0, nrays, ncrossings, 2);
              int* boxz = vec4D(boxes, beam,raynum,numcrossing,1, nrays, ncrossings, 2);
              *boxx = thisx + 1;//[beam][raynum][numcrossing][0]
              *boxz = thisz + 1;//[beam][raynum][numcrossing][1]
            }
            lastx = currx;
            numcrossing += 1;
            break;
