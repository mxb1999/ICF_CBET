...
cbetUpdate(int nbeams, int nrays, int ncrossings, double* wMult, double* intensity, int* boxes)
{
    ...
    {
      int ix = vec4D_cu(boxes, beam, raynum, m, 0, nrays, ncrossings, 2);
      int iz = vec4D_cu(boxes, beam, raynum, m, 1, nrays, ncrossings, 2);
      if(!ix || !iz)
      {
        break;
      }
      ix = vec4D_cu(boxes, beam, raynum, m+1, 0, nrays, ncrossings, 2);
      iz = vec4D_cu(boxes, beam, raynum, m+1, 1, nrays, ncrossings, 2);
      if(!ix || !iz)
      {
        break;
      }
